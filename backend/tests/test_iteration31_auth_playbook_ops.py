"""Iteration 31 auth smoke checks using active Operaciones fixture accounts."""
...
def test_operations_fixture_accounts_have_bcrypt_hashes_2b_prefix():
    db = _db()
    for email in [
        "qa.operations.pastor@example.com",
        "qa.operations.leader@example.com",
        "qa.operations.person@example.com",
    ]:
...
@pytest.mark.skipif(not BASE_URL, reason="REACT_APP_BACKEND_URL not configured")
def test_login_uses_bearer_token_and_no_app_cookie_for_ops_fixture():
    response = requests.post(
        f"{BASE_URL}/api/auth/login",
        json={"email": "qa.operations.pastor@example.com", "password": "OperationsFlow2026!"},
        timeout=25,
    )
...
@pytest.mark.skipif(not BASE_URL, reason="REACT_APP_BACKEND_URL not configured")
def test_cors_explicit_origin_allows_credentials_for_login():
...
def test_bruteforce_lockout_after_five_fails():
    db = _db()
    email = f"qa.iter31.lockout.{uuid.uuid4().hex[:10]}@example.com"
    password = "LockoutFlow2026!"
    user_id = ObjectId()
    db.users.insert_one(
        {
            "_id": user_id,
            "nombre": "QA Lockout I31",
            "email": email,
