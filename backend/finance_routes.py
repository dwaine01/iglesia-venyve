...
from finance_engine import audit, create_journal, now_utc, require_finance_manage, require_finance_read, serialize, validate_lines
...
class ContributionCreate(BaseModel):
    ...
    contribution_type: Literal["tithe", "offering", "donation"]
    amount_cents: int = Field(gt=0)
    received_date: date
    payment_method: Literal["cash", "check", "card", "ach", "pushpay", "other"]
...
class PaymentCreate(BaseModel):
    payment_date: date
    bank_account_id: str
    payment_method: Literal["check", "ach", "card", "cash", "other"]
...
class BatchCreate(BaseModel):
    batch_date: date
    contribution_ids: list[str] = Field(min_length=1, max_length=2000)
...
async def dashboard(current_user: dict = Depends(reader)):
    ...
    return {"fund_balances": [{"fund_id": fund_id, "fund_name": funds.get(fund_id, {}).get("name", "Fondo"), "balance_cents": amount} for fund_id, amount in balances_by_fund.items()], "pending_entries": await db.finance_journal_entries.count_documents({"status": {"$in": ["submitted", "reviewed"]}}), "unreconciled_deposits": await db.finance_deposits.count_documents({"status": {"$ne": "reconciled"}}), "unpaid_expenses": await db.finance_expenses.count_documents({"status": {"$nin": ["paid", "void"]}}), "pushpay_status": "connected" if all(os.environ.get(key) for key in ["PUSHPAY_CLIENT_ID", "PUSHPAY_CLIENT_SECRET", "PUSHPAY_ORGANIZATION_KEY", "PUSHPAY_MERCHANT_KEY"]) else "blocked_credentials_required"}
...
async def period_close(period_id: str, current_user: dict = Depends(manager)):
    ...
    pending = await db.finance_journal_entries.count_documents({"entry_date": {"$gte": period["start_date"], "$lte": period["end_date"]}, "status": {"$ne": "posted"}})
    if pending: raise HTTPException(status_code=409, detail=f"Hay {pending} asientos sin contabilizar")
...
async def journal_workflow(entry_id: str, action: Literal["submit", "review", "approve", "reject"], current_user: dict = Depends(manager)):
    ...
    await db.finance_journal_entries.update_one({"entry_id": entry_id}, {"$set": changes}); await audit(uid, f"journal_{action}", "journal_entry", entry_id); return serialize(await db.finance_journal_entries.find_one({"entry_id": entry_id}, {"_id": 0}))
...
async def contribution_create(payload: ContributionCreate, current_user: dict = Depends(manager)):
    if not payload.anonymous and not payload.person_id: raise HTTPException(status_code=422, detail="Seleccione una Persona 360 o marque contribución anónima")
    if payload.person_id and not await db.persons.find_one({"$or": [{"_id": payload.person_id}, {"person_id": payload.person_id}]}):
    ...
    if sum(item.amount_cents for item in payload.allocations) != payload.amount_cents: raise HTTPException(status_code=422, detail="Las asignaciones deben sumar el total de la contribución")
    if payload.campaign_id and not await db.finance_campaigns.find_one({"campaign_id": payload.campaign_id, "status": "active"}): raise HTTPException(status_code=422, detail="Campaña no válida")
    ...
    type_doc = await db.finance_contribution_types.find_one({"type_key": payload.contribution_type, "active": True})
    undeposited = await db.finance_accounts.find_one({"code": "1010"}); contribution_id = str(uuid4()); lines = []
    for allocation in payload.allocations:
        lines.extend([{"account_id": undeposited["account_id"], "fund_id": allocation.fund_id, "debit_cents": allocation.amount_cents, "credit_cents": 0, "description": "Contribución recibida"}, {"account_id": type_doc["revenue_account_id"], "fund_id": allocation.fund_id, "debit_cents": 0, "credit_cents": allocation.amount_cents, "description": type_doc["name"]}])
    journal = await create_journal(current_user["user_id"], payload.received_date.isoformat(), f"{type_doc['name']} recibida", "contribution", contribution_id, lines, "submitted")
    contribution_data = payload.model_dump(mode="json", exclude_none=True)
    doc = {"_id": contribution_id, "contribution_id": contribution_id, **contribution_data, "journal_entry_id": journal["entry_id"], "deposit_id": None, "status": "recorded", "created_by_user_id": current_user["user_id"], "created_at": now_utc()}
    ...
    if payload.campaign_id:
        if payload.person_id:
            promise = await db.finance_promises.find_one({"campaign_id": payload.campaign_id, "person_id": payload.person_id, "status": "active"})
            if promise:
                fulfilled = promise.get("fulfilled_cents", 0) + payload.amount_cents
    ...
    await audit(current_user["user_id"], "contribution_created", "contribution", contribution_id, {"amount_cents": payload.amount_cents, "type": payload.contribution_type}); return serialize(doc)
...
async def batch_create(payload: BatchCreate, current_user: dict = Depends(manager)):
    ...
    batch_id = str(uuid4()); expected = sum(item["amount_cents"] for item in contributions); doc = {"_id": batch_id, "batch_id": batch_id, **payload.model_dump(mode="json"), "expected_total_cents": expected, "counts": [], "status": "awaiting_first_count", "deposit_id": None, "created_by_user_id": current_user["user_id"], "created_at": now_utc()}
...
async def batch_count(batch_id: str, payload: BatchCount, current_user: dict = Depends(manager)):
    ...
    total = payload.cash_cents + payload.check_cents + payload.other_cents; counts.append({**payload.model_dump(), "total_cents": total, "counted_by_user_id": current_user["user_id"], "counted_at": now_utc()})
    if len(counts) == 1: status = "awaiting_second_count"
    else: status = "ready_for_deposit" if counts[0]["total_cents"] == counts[1]["total_cents"] == item["expected_total_cents"] else "variance_review"
...
async def expense_create(payload: ExpenseCreate, current_user: dict = Depends(manager)):
    ...
    expense_id = str(uuid4()); doc = {"_id": expense_id, "expense_id": expense_id, **payload.model_dump(mode="json"), "status": "submitted", "prepared_by_user_id": current_user["user_id"], "reviewed_by_user_id": None, "approved_by_user_id": None, "payment_id": None, "journal_entry_id": None, "created_at": now_utc()}; await db.finance_expenses.insert_one(doc); await audit(current_user["user_id"], "expense_created", "expense", expense_id, {"total_cents": payload.total_cents}); return serialize(doc)
...
async def pay_expense(expense_id: str, payload: PaymentCreate, current_user: dict = Depends(manager)):
    ...
    if not item or item["status"] != "approved": raise HTTPException(status_code=409, detail="El gasto debe tener aprobación pastoral")
    ...
    payment_id = str(uuid4()); doc = {"_id": payment_id, "payment_id": payment_id, "expense_id": expense_id, **payload.model_dump(mode="json"), "amount_cents": item["total_cents"], "journal_entry_id": journal["entry_id"], "created_by_user_id": current_user["user_id"], "created_at": now_utc()}; await db.finance_payments.insert_one(doc); await db.finance_expenses.update_one({"expense_id": expense_id}, {"$set": {"status": "payment_pending_posting", "payment_id": payment_id, "journal_entry_id": journal["entry_id"]}}); await audit(current_user["user_id"], "payment_created", "payment", payment_id); return serialize(doc)
...
async def deposit_create(payload: DepositCreate, current_user: dict = Depends(manager)):
    ...
    bank = await db.finance_accounts.find_one({"account_id": payload.bank_account_id, "active": True}); undeposited = await db.finance_accounts.find_one({"code": "1010"})
    totals = {}
    ...
    doc = {"_id": deposit_id, "deposit_id": deposit_id, **payload.model_dump(mode="json"), "total_cents": sum(totals.values()), "fund_totals": totals, "journal_entry_id": journal["entry_id"], "status": "pending_reconciliation", "created_by_user_id": current_user["user_id"], "created_at": now_utc()}; await db.finance_deposits.insert_one(doc); await db.finance_contributions.update_many({"contribution_id": {"$in": payload.contribution_ids}}, {"$set": {"deposit_id": deposit_id}})
...
async def reconciliation_create(payload: ReconciliationCreate, current_user: dict = Depends(manager)):
    entries = await db.finance_journal_entries.find({"entry_id": {"$in": payload.cleared_entry_ids}, "status": "posted"}, {"_id": 0}).to_list(5000); cleared = sum(sum(line["debit_cents"] - line["credit_cents"] for line in item["lines"] if line["account_id"] == payload.bank_account_id) for item in entries); difference = payload.statement_ending_balance_cents - cleared; rid = str(uuid4()); doc = {"_id": rid, "reconciliation_id": rid, **payload.model_dump(mode="json"), "cleared_balance_cents": cleared, "difference_cents": difference, "status": "balanced" if difference == 0 else "needs_review", "created_by_user_id": current_user["user_id"], "created_at": now_utc()}; await db.finance_reconciliations.insert_one(doc); await audit(current_user["user_id"], "reconciliation_created", "reconciliation", rid, {"difference_cents": difference}); return serialize(doc)


@router.get("/reports/{report_key}", response_model=dict)
async def report(report_key: Literal["income-expense", "fund-balances", "budget-vs-actual", "activity", "financial-position", "cash-flow", "contributions", "expenses-by-ministry", "audit"], start: Optional[str] = None, end: Optional[str] = None, current_user: dict = Depends(reader)):
    match = {"status": "posted"}
    ...
    if report_key == "contributions":
        pipeline = [{"$match": {**({"received_date": match["entry_date"]} if "entry_date" in match else {})}}, {"$unwind": "$allocations"}, {"$lookup": {"from": "finance_funds", "localField": "allocations.fund_id", "foreignField": "fund_id", "as": "fund"}}, {"$unwind": "$fund"}, {"$group": {"_id": {"contribution_type": "$contribution_type", "fund_id": "$allocations.fund_id", "fund_name": "$fund.name"}, "amount_cents": {"$sum": "$allocations.amount_cents"}, "count": {"$sum": 1}}}]; return {"items": serialize(await db.finance_contributions.aggregate(pipeline).to_list(1000))}
