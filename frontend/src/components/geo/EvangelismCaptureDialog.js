...
const emptyForm = { house_number: '', street_name: '', address2: '', city: 'Columbus', state: 'OH', zip: '', language: 'unknown', notes: '', assigned_to_user_id: '' };

export const EvangelismCaptureDialog = ({ open, onOpenChange, onCreated }) => {
  const { API, getAuthHeaders } = useAuth(); const [form, setForm] = useState(emptyForm);
  ...
  const submit = async (event) => {
    ...
    try { const response = await axios.post(`${API}/api/geo/evangelism`, { ...form, address2: form.address2 || null, zip: form.zip || null, notes: form.notes || null, assigned_to_user_id: form.assigned_to_user_id || null }, getAuthHeaders()); toast.success(response.data.geocoding_status === 'matched' ? 'Casa registrada y ubicada en el mapa' : 'Casa registrada; ubicación pendiente de verificación'); setForm(emptyForm); onOpenChange(false); onCreated?.(response.data); }
    catch (requestError) { setError(apiErrorMessage(requestError, 'No se pudo registrar la casa')); }
    finally { setSaving(false); }
  };
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[92vh] max-w-2xl overflow-y-auto bg-white" data-testid="evangelism-capture-dialog"><DialogHeader><DialogTitle className="flex items-center gap-2 font-['Spectral'] text-2xl"><Home className="h-5 w-5 text-emerald-700" />Registrar casa por visitar</DialogTitle></DialogHeader><form onSubmit={submit} className="space-y-5" data-testid="evangelism-capture-form">
