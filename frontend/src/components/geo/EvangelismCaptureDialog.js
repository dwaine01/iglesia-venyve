...
export const EvangelismCaptureDialog = ({ open, onOpenChange, onCreated, frontGroup = null }) => {
  ...
  const submit = async (event) => {
    ...
    try { const response = await axios.post(`${API}/api/geo/evangelism`, { ...form, address2: form.address2 || null, zip: form.zip || null, notes: form.notes || null, pastoral_notes: !frontGroup && canUsePastoralNotes && form.pastoral_notes ? form.pastoral_notes : null, assigned_to_user_id: form.assigned_to_user_id || null, front_group_id: frontGroup?.front_group_id || null, invasion_reference: frontGroup ? `INV-${frontGroup.front_group_id.slice(0, 8)}-${new Date().toISOString().slice(0, 10)}` : null }, getAuthHeaders()); toast.success(response.data.geocoding_status === 'matched' ? 'Casa registrada y ubicada en el mapa' : 'Casa registrada; ubicación pendiente de verificación'); setForm(emptyForm); onOpenChange(false); onCreated?.(response.data); }
    catch (requestError) { setError(apiErrorMessage(requestError, 'No se pudo registrar la casa')); }
    finally { setSaving(false); }
  };
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[92vh] max-w-2xl overflow-y-auto bg-white" data-testid="evangelism-capture-dialog"><DialogHeader><DialogTitle className="flex items-center gap-2 font-['Spectral'] text-2xl"><Home className="h-5 w-5 text-emerald-700" />{frontGroup ? 'Crear invasión' : 'Registrar casa por visitar'}</DialogTitle><DialogDescription>{frontGroup ? `${frontGroup.name} quedará registrado como Grupo originador en el Mapa 360.` : 'Registre la dirección operativa y asigne la primera visita. Las notas pastorales tienen acceso restringido.'}</DialogDescription></DialogHeader><form onSubmit={submit} className="space-y-5" data-testid="evangelism-capture-form">
    ...
    {!frontGroup && canUsePastoralNotes && <div><Label htmlFor="ev-pastoral-notes">Notas pastorales restringidas</Label><Textarea id="ev-pastoral-notes" value={form.pastoral_notes} onChange={update('pastoral_notes')} rows={3} placeholder="Visible únicamente para autoridad pastoral" data-testid="evangelism-pastoral-notes-input" /></div>}
    <div className="flex justify-end gap-2 border-t pt-4"><Button type="button" variant="outline" onClick={() => onOpenChange(false)} data-testid="evangelism-capture-cancel-button">Cancelar</Button><Button type="submit" disabled={saving} className="bg-emerald-700 hover:bg-emerald-800" data-testid="evangelism-capture-submit-button">{saving && <Loader2 className="h-4 w-4 animate-spin" />}{frontGroup ? 'Crear invasión' : 'Registrar casa'}</Button></div>
