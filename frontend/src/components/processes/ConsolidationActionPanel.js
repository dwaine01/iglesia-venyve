...
export const ConsolidationActionPanel = ({ journey, assignees, onUpdated }) => {
  ...
  const toggleTask = (task, checked) => call(task.task_id, () => axios.put(`${API}/api/processes/enrollments/${journey.enrollment_id}/stages/${current.stage_key}/tasks/${task.task_id}`, { completed: checked }, getAuthHeaders()));
  ...
  const canAccept = user?.rol === 'pastor' || capabilities.includes('membership.acceptance.manage');
  const canTransfer = user?.rol === 'pastor' || capabilities.includes('consolidation.mentor.transfer');
  const canCloseRetreat = user?.rol === 'pastor' || capabilities.includes('consolidation.retreat.close');
