...
import { displayLabel, stageLabel, statusLabel } from '../lib/displayLabels';
...
export function HouseholdSection({ personId, record, canWrite, API, getAuthHeaders, onChanged }) {
  ...
  const save = async (event) => {
    ...
    try {
      ...
      setMessage('Hogar actualizado'); await onChanged();
    } catch (err) { setMessage(err?.response?.data?.detail || 'No se pudo guardar.'); } finally { setSaving(false); }
  };
  return (
    <Shell title="Hogar" description="Información del hogar actual, separada de las relaciones familiares." icon={House}>
      <form onSubmit={save} className="grid gap-4 rounded-xl border bg-white p-4 sm:grid-cols-2">
        ...
        <div className="flex items-center justify-between sm:col-span-2">{message && <p className="text-sm text-gray-600" data-testid="household-save-message">{message}</p>}{canWrite && <Button disabled={saving} className="ml-auto bg-[#132443]" data-testid="save-household-button">{saving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Guardar hogar</Button>}</div>
      </form>
...
export function FamilySection({ personId, items = [], canWrite, API, getAuthHeaders, onChanged }) {
  ...
  return (
    <Shell title="Familia" description="Familia inmediata y extendida, vinculada al mismo Perfil 360." icon={UsersRound}>
...
export function ProcessesSection({ personId, arrival, processes = [], ministries = [], canWrite, canWriteMinistries, API, getAuthHeaders, onChanged }) {
  ...
  return (
    <div className="space-y-7">
      ...
      <Shell title="Ministerios" description="Asignaciones ministeriales vinculadas al mismo Perfil 360." icon={UsersRound}>
      ...
      <Shell title="Procesos" description="Conectores preparados; cada módulo conserva su propia fuente de verdad." icon={Clock3}>
        {processes.length === 0 ? <div className="rounded-md border border-dashed p-10 text-center text-sm text-gray-500" data-testid="profile-processes-empty">Sin procesos registrados para esta Persona.</div> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{processes.map((item) => <Link key={item.enrollment_id} to={item.process_key === 'seven_weeks' ? `/procesos/7-semanas/${item.enrollment_id}` : item.route} className="rounded-md border bg-white p-4 transition hover:border-[#C8A951] hover:shadow-sm" data-testid={`profile-process-${item.enrollment_id}`}><div className="flex items-center justify-between gap-2"><p className="font-semibold text-[#101D36]">{displayLabel(item.process_key, item.label || 'Proceso')}</p><Badge variant="outline" className="text-gray-600">{statusLabel(item.status_label)}</Badge></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-gray-100"><div className="h-full bg-[#C8A951]" style={{ width: `${item.progress_pct || 0}%` }} /></div><p className="mt-2 text-sm text-gray-600">{stageLabel(item.current_stage_key)}</p><p className="mt-1 text-xs text-gray-500">{item.next_action || 'Sin próxima acción'}</p></Link>)}</div>}
      </Shell>
    </div>
  );
}
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-2">
      <Shell title="Notas" description="Notas internas autorizadas sobre esta persona." icon={NotebookPen}>
        ...
        <div className="space-y-2">{notes.map((note) => <div key={note.note_id} className="rounded-xl border bg-white p-4"><div className="flex items-start gap-3"><FileText className="mt-0.5 h-5 w-5 text-[#755B21]" /><div className="flex-1"><Badge variant="outline" className="mb-2 capitalize">{displayLabel(note.categoria)}</Badge><p className="whitespace-pre-wrap text-sm text-gray-700">{note.contenido}</p><p className="mt-2 text-xs text-gray-400">{new Date(note.created_at).toLocaleString('es')}</p></div>{canWrite && <Button variant="ghost" size="icon" onClick={() => removeNote(note.note_id)} className="text-red-600" aria-label="Eliminar nota" data-testid={`delete-note-${note.note_id}`}><Trash2 className="h-4 w-4" /></Button>}</div></div>)}</div>
      </Shell>
      <Shell title="Historial de actividad" description="Cambios reales registrados por los dominios del perfil." icon={Clock3}>
        <div className="space-y-0">{activity.length === 0 ? <div className="rounded-xl border border-dashed p-10 text-center text-sm text-gray-500">Sin actividad registrada todavía.</div> : activity.map((item, index) => <div key={item.activity_id} className="relative flex gap-3 pb-5"><div className="relative z-10 mt-1 h-3 w-3 shrink-0 rounded-full bg-[#C8A951]" />{index < activity.length - 1 && <div className="absolute left-[5px] top-4 h-full w-px bg-gray-200" />}<div><p className="font-medium text-[#101D36]">{item.summary}</p><p className="text-xs text-gray-500">{displayLabel(item.domain, 'Actividad')} · {new Date(item.created_at).toLocaleString('es')}</p></div></div>)}</div>
      </Shell>
    </div>
  );
}
