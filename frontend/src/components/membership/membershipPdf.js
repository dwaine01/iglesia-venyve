...
const printFrame = async ({ documentType, elements }) => {
  const isCertificate = documentType === 'certificate';
  const size = isCertificate ? '11in 8.5in' : '85.6mm 53.98mm';
  ...
  printDocument.open();
  printDocument.write(`<!doctype html><html><head><meta charset="utf-8"><base href="${document.baseURI}">${styles}<style>@page{size:${size};margin:0}html,body{margin:0!important;padding:0!important;background:#fff!important}.vv-print-page{width:${isCertificate ? '11in' : '85.6mm'};height:${isCertificate ? '8.5in' : '53.98mm'};margin:0;overflow:hidden;break-after:page;page-break-after:always}.vv-print-page:last-child{break-after:auto;page-break-after:auto}.vv-print-page>[data-vv-document]{margin:0!important;box-shadow:none!important}</style></head><body>${pages}</body></html>`);
  printDocument.close();
...
export const printMembershipDocument = async ({ documentType, certificate, cardFront, cardBack }) => {
  const elements = documentType === 'certificate' ? [certificate] : [cardFront, cardBack];
  if (elements.some((element) => !element)) throw new Error('Documento no disponible para impresión');
  await printFrame({ documentType, elements });
};
