...
export const MembershipDocumentDialog = ({ open, onOpenChange, documentType, data, photoSrc, signatureSrc }) => {
  ...
  if (!data) return null;
  const previewBaseWidth = documentType === 'certificate' ? 1000 : 760;
  const shellStyle = { width: `${zoom * 100}%`, maxWidth: `${previewBaseWidth * zoom}px` };
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[94vh] max-w-6xl overflow-y-auto bg-[#F4F6F7]" data-testid="membership-document-preview-dialog">
        <DialogHeader><DialogTitle>{documentType === 'certificate' ? 'Certificado oficial de membresía' : 'Carnet oficial de miembro'}</DialogTitle></DialogHeader>
        ...
        <div className="membership-document-scroll">
          {documentType === 'certificate' ? (
            ...
            <Tabs defaultValue="front">
              ...
              <TabsContent value="front"><div className="membership-preview-zoom-shell card" style={shellStyle}><MembershipCardFront data={data} photoSrc={photoSrc} /></div></TabsContent>
              <TabsContent value="back"><div className="membership-preview-zoom-shell card" style={shellStyle}><MembershipCardBack data={data} qrSrc={qrSrc} /></div></TabsContent>
            </Tabs>
          )}
        </div>
        ...
        <div className="membership-export-stage" aria-hidden="true">
          {documentType === 'certificate' ? <div ref={certificateRef} className="membership-export-certificate"><MembershipCertificateTemplate data={data} signatureSrc={signatureSrc} qrSrc={qrSrc} exportMode /></div> : <><div ref={cardFrontRef} className="membership-export-card"><MembershipCardFront data={data} photoSrc={photoSrc} exportMode /></div><div ref={cardBackRef} className="membership-export-card"><MembershipCardBack data={data} qrSrc={qrSrc} exportMode /></div></>}
        </div>
      </DialogContent>
    </Dialog>
  );
