...
export const MembershipCertificateTemplate = ({ data, signatureSrc, qrSrc, exportMode = false }) => {
  ...
  return (
    <article id={rootId} data-vv-document="certificate" style={{ ...membershipDocumentTokens, boxShadow: exportMode ? 'none' : undefined }} data-testid={testIdFor('membership-certificate-template', exportMode)}>
      ...
      <h2 id={`${rootId}-name`} data-vv-role="member-name" style={{ fontSize: memberNameSize(person.full_name, 'certificate'), whiteSpace: wrapsName ? 'normal' : 'nowrap', height: wrapsName ? '1in' : '.72in', lineHeight: wrapsName ? .92 : 1, display: wrapsName ? '-webkit-box' : 'block', WebkitLineClamp: wrapsName ? 3 : undefined, WebkitBoxOrient: wrapsName ? 'vertical' : undefined, overflow: 'hidden', textOverflow: 'ellipsis' }} data-testid={testIdFor('certificate-member-name', exportMode)}>{person.full_name}</h2>
