...
export const FrontGroupDetailPanel = ({ group, canManage, onUpdated }) => {
  ...
  const load = async () => {
    const [groupResponse, peopleResponse] = await Promise.all([axios.get(`${API}/api/front-groups/${group.front_group_id}`, getAuthHeaders()), axios.get(`${API}/api/core/persons?limit=500`, getAuthHeaders())]);
    setDetail(groupResponse.data); setPeople(peopleResponse.data.items || []);
  };
  useEffect(() => { load().catch(() => setDetail(null)); }, [group.front_group_id]); // eslint-disable-line react-hooks/exhaustive-deps
  const act = async (key, request) => {
    ...
    catch (error) { toast.error(error?.response?.data?.detail || 'No se pudo actualizar el Grupo Frontal'); }
    finally { setBusy(''); }
  };
  if (!detail) return <div className="flex min-h-64 items-center justify-center"><Loader2 className="h-6 w-6 animate-spin" /></div>;
