...
const delegatedCapabilities = [
  ...
  ['front_groups.view', 'Consultar Grupos Frontales'],
  ['front_groups.manage', 'Administrar Grupos Frontales'],
  ['leadership.view', 'Consultar Liderazgo'],
...
export const CoreAccessTable = ({ items = [], currentUser, saving, onSave }) => {
  ...
  return <section className="overflow-hidden border bg-white" data-testid="core-access-table">
    ...
    <div className="overflow-x-auto"><Table><TableHeader><TableRow>
      ...
      const locked = item.user_id === (currentUser?.user_id || currentUser?.id);
      return <TableRow key={item.user_id} data-testid={`access-row-${item.user_id}`}>
        ...
        <TableCell><div className="flex max-w-xs flex-wrap gap-2">{groups.map((group) => <label key={group} className="flex items-center gap-1 text-xs"><Checkbox disabled={locked || pastorOnlyGroups.has(group) && !isPastoralAuthority(currentUser)} checked={(draft.privilege_groups || []).includes(group)} onCheckedChange={(checked) => patch(item.user_id, { privilege_groups: toggle(draft.privilege_groups, group, checked) })} data-testid={`access-group-${group}-${item.user_id}`} />{displayLabel(group)}</label>)}</div></TableCell>
        <TableCell><div className="grid min-w-[270px] gap-2">{delegatedCapabilities.map(([capability, label]) => <label key={capability} className="flex items-center gap-2 text-xs"><Checkbox disabled={locked || !isPastoralAuthority(currentUser)} checked={(draft.capabilities || []).includes(capability)} onCheckedChange={(checked) => patch(item.user_id, { capabilities: toggle(draft.capabilities, capability, checked) })} data-testid={`access-capability-${capability.replaceAll('.', '-').replaceAll('_', '-')}-${item.user_id}`} /><ShieldCheck className="h-3.5 w-3.5 text-[#B5953F]" />{label}</label>)}</div></TableCell>
