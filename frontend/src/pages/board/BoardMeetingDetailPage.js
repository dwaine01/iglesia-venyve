...
export default function BoardMeetingDetailPage() {
  ...
  const actions = meeting?.status === 'open' ? <div className="flex flex-wrap gap-2"><Button onClick={() => setActiveTab('recording')} className="bg-red-700 hover:bg-red-800" data-testid="go-to-recording-button"><Mic className="mr-2 h-4 w-4" />Ir a grabación</Button><Button onClick={close} variant="outline" data-testid="close-board-meeting-button"><DoorClosed className="mr-2 h-4 w-4" />Cerrar reunión</Button></div> : null;
