...
export default function FrontGroupsPage() {
  ...
  const load = useCallback(async () => {
    ...
    catch (requestError) { setError(apiErrorMessage(requestError, 'No se pudieron cargar los Grupos Frontales')); }
  }, [API, getAuthHeaders]);
  useEffect(() => { load(); }, [load]);
  const canManage = hasCapability(user, 'front_groups.manage');
