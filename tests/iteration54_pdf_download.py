...
OUT = Path("/app/test_reports/pdf_bug_verification2")
...
async def download_cert(name_tag: str):
    async with async_playwright() as pw:
        ...
        # Go to persona 360
        await page.goto(f"{BASE}/personas/{STATE['member_person_id']}")
        await page.wait_for_timeout(3000)
        print("Persona url=", page.url)
        await page.screenshot(path=str(OUT/f"persona_{name_tag}.png"), quality=40, full_page=False)

        # Click Membresía tab
        try:
            await page.get_by_role("tab", name="Membresía").click(timeout=5000)
        except Exception:
            # try button/text
            await page.get_by_text("Membresía", exact=False).first.click(force=True)
        ...
        await page.screenshot(path=str(OUT/f"dialog_{name_tag}.png"), quality=40, full_page=False)
